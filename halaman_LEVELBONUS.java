...
public class halaman_LEVELBONUS extends World {
    private LabelSoal labelSoal;

    private Player player;
    private monster6 monster;
    ...
    private void prosesJawaban(String input) {
        ...
        showText("", getWidth() / 2, getHeight() / 2 - 50);
            ...
            if (input.equals("")) {
             showText("Waktu habis!", getWidth() / 2, getHeight() / 2 - 50);
             Greenfoot.delay(40);
            soalIndex++;
            tampilkanPertanyaan();
            return;
        ...
        if (input.equalsIgnoreCase(jawabanBenar)) {
            addObject(new peluru_apibonus(monster), player.getX(), player.getY());
            nyawaMonster--;
          ...
          GlobalData.selesaikanLevel("LEVELBONUS");
          Greenfoot.delay(60);
    ...
    private void prepare()
    {
        ...
        monster = new monster6();

        addObject(new KE_AWAL(), 42, 41);

        monster6 monster6 = new monster6();
        addObject(monster6,569,289);
        KE_HOME kE_HOME = new KE_HOME();
        addObject(kE_HOME,842,54);
    }
}
